...
# Shared format
FMT="%-10s %-18s %-20s %-12s %-12s %-12s %-6s %-6s\n"

# Print header
printf "${BOLD}${CYAN}${FMT}${RESET}" \
  "TIME" "SERVER_IP" "DB_HOST" "PRIMARY" "REPLICA_1" "REPLICA_2" "LAG_R1" "LAG_R2"
...
while true; do
  ...
  IFS=$'\t' read -r SERVER_IP DB_HOST P_ST R1_ST R2_ST R1_LAG R2_LAG <<< "$LINE"
  ...
  printf "%-10s %-18s %-20s %b %b %b %-6s %-6s\n" \
    "$(date '+%H:%M:%S')" "$SERVER_IP" "$DB_HOST" "$P_FMT" "$R1_FMT" "$R2_FMT" "$R1_LAG" "$R2_LAG"
